from bioterms.etc.enums import ConceptPrefix
from .hgnc import HGNC_CONCEPT
from .uniprot import UNIPROT_CONCEPT
from .utils import resolve_concept_annotated_concepts


@HGNC_CONCEPT.field('annotatedUniProt')
async def resolve_hgnc_concept_annotated_uniprot(obj, info):
    return await resolve_concept_annotated_concepts(
        obj=obj, info=info, source_prefix=ConceptPrefix.HGNC,
        target_prefix=ConceptPrefix.UNIPROT,
    )


@UNIPROT_CONCEPT.field('annotatedHgnc')
async def resolve_uniprot_concept_annotated_hgnc(obj, info):
    return await resolve_concept_annotated_concepts(
        obj=obj, info=info, source_prefix=ConceptPrefix.UNIPROT,
        target_prefix=ConceptPrefix.HGNC,
    )
