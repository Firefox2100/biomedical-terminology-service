"""Resolvers for UniProtKB concepts and queries."""
...
UNIPROT_QUERY = ObjectType('UniProtQuery')
...
async def resolve_uniprot_concept_similar_concepts(obj, info, threshold: float = 1.0):
...
@UNIPROT_CONCEPT.field('pathsTo')
async def resolve_uniprot_concept_paths_to(obj, info, target_prefix: str,
...
@UNIPROT_QUERY.field('uniprotConcept')
async def resolve_get_uniprot_concept(_, info, concept_id: str) -> dict:
...
@GRAPHQL_QUERY_TYPE.field('uniprot')
async def resolve_uniprot_query(_, __) -> dict:
    return {}
