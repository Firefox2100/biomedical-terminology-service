...
ANNOTATION_NAME = 'HGNC Mapping to OMIM Gene'
...
async def download_annotation(download_client: httpx.AsyncClient = None):
    """Reuse the complete HGNC release containing OMIM-provided cross-references."""
    await download_vocabulary(download_client=download_client)


async def load_annotation_from_file(graph_db: GraphDatabase = None):
    """Load the OMIM gene cross-references published in the HGNC release."""
    ...
    annotations = load_hgnc_mapping(
        'omim_id', ConceptPrefix.OMIM, AnnotationType.EXACT,
    )
    verbose_print(f'Processed {len(annotations)} HGNC to OMIM annotations. Saving...')
