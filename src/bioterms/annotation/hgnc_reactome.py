...
ANNOTATION_NAME = 'Reactome ReferenceEntity Mapping to HGNC'
VOCABULARY_PREFIX_1 = ConceptPrefix.REACTOME
VOCABULARY_PREFIX_2 = ConceptPrefix.HGNC
FILE_PATHS = ['reactome/hgnc_mapping.csv']
...
async def load_annotation_from_file(graph_db: GraphDatabase = None):
    ...
    # HGNC is a gene-vocabulary link, so it intentionally has no source provenance property.
    annotations = build_reference_annotations(ConceptPrefix.HGNC, source_name=None)
    verbose_print(f'Processed {len(annotations)} Reactome to HGNC annotations. Saving...')
