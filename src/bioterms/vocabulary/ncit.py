...
async def download_vocabulary(download_client: httpx.AsyncClient = None):
    ...
    with zipfile.ZipFile(
        str(os.path.join(CONFIG.data_dir, flat_file_path))
    ) as zip_ref:
        zip_ref.extractall(str(os.path.join(CONFIG.data_dir, 'ncit')))

    try:
        os.remove(str(os.path.join(CONFIG.data_dir, flat_file_path)))
    except Exception:
        pass
